...
import { Check, ChevronDown, Download, Grid2X2, Maximize2, ScanSearch, Share2, SlidersHorizontal, Sparkles, UserRound } from 'lucide-react';
...
export function PhotoWorkspace({ workspace, preset, busy, error, capabilities, onUpscale, onBack, onChangeMedia, onSave, onGenerate, onRefine, onCancel, onRefresh, onDetect }: V4WorkspaceProps) {
  ...
  const outputCount = Math.ceil(selected.length / bracketSize) * arrangements;
  ...
  const blocked = busy || running || selected.length === 0 || invalidBrackets || !availability.ready;
  ...
  const settings = <>
    ...
    <InspectorSection title="Property details"><PhotoServiceControls config={config} adjust={adjust} disabled={busy || running} /></InspectorSection>
    <InspectorSection title="Make it yours"><p>Guide the AI edit. Changes appear in the generated result.</p>{[['brightness', 'Brightness', -30, 30, 0], ['warmth', 'Warmth', -30, 30, 0], ['windows', 'Window recovery', 0, 100, 50]].map(([key, label, min, max, initial]) => <label className="v4-slider-field" key={String(key)}><span>{label}<output>{Number(config.adjustments[String(key)] ?? initial)}</output></span><input type="range" min={Number(min)} max={Number(max)} value={Number(config.adjustments[String(key)] ?? initial)} onChange={event => adjust(String(key), Number(event.target.value))} /></label>)}
      ...
      {fullShoot && <label className="v4-field">RAW processing<select value={1} disabled><option value={1}>Enhance each image</option><option value={3} disabled>3-exposure merge · unavailable</option><option value={5} disabled>5-exposure merge · unavailable</option></select><span>Each source creates one edited photo. Bracket merging is not available in this workflow.</span></label>}
    </InspectorSection>
