...
import type { StudioRatio, V4Frame, V4Media, V4WorkspaceProps } from '@/components/studio/v4/types';
...
export function VideoWorkspace({ workspace, preset, busy, error, capabilities, onBack, onChangeMedia, onSave, onPrepare, onGenerate, onRefine, onCancel, onRefresh, onDetect }: V4WorkspaceProps) {
  ...
  const generationLabel = workspace.status === 'preparing' ? 'Preparing frames' : generation?.phase === 'rendering' ? 'Rendering video' : 'Generating video';
  const generationCopy = workspace.status === 'preparing' ? `${readyFrames.length} of ${frames.length} frames ready`
    : generation ? `${generation.completed} of ${generation.total} clips ready${generation.phase === 'rendering' ? '' : ` · ${generation.submitted} submitted`}` : undefined;
  ...
  const canPrepare = capabilities?.outpaint.ready !== false || !frames.some(frame => frame.method === 'extend');
  const canGenerate = capabilities?.presets[preset.id]?.ready !== false;
  ...
  if (!active) return <div className="v4-workspace-empty"><EmptyState icon="videos" title={<>Choose images for your video</>} size="compact" /><p>Start with property photos from a shoot or your uploads.</p><Button onClick={onChangeMedia}>Select media</Button></div>;
