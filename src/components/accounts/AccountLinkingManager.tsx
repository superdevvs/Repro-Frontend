    ...
    <div className="space-y-6">
      <section className="rounded-lg border bg-card p-4 text-card-foreground shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted/50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground"><Link2 className="h-3.5 w-3.5" /> Linking Workspace</div>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Review active groups, reactivate broken relationships, and update shared access without the old batch-only flow.</p>
          </div>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
          ...
          ].map(([label, value, note], index) => <div key={String(label)} className="rounded-2xl border border-border/60 bg-muted/30 p-3.5"><p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">{label}</p><p className={cn('mt-2 text-2xl font-semibold tracking-tight', index === 2 ? 'text-amber-600 dark:text-amber-300' : 'text-foreground')}>{value}</p><p className="mt-1 text-sm text-muted-foreground">{note}</p></div>)}
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="rounded-lg border bg-card p-4 text-card-foreground shadow-sm">
          ...
          <div className="mt-4">
            {loading ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-2xl" />)}</div> : groups.length === 0 ? <div className="rounded-2xl border border-dashed p-6 text-center"><EmptyState icon="linked" title={<>No linked groups yet</>} size="compact" /><Button className="mt-4" onClick={() => openCreate()} disabled={!canUpdate}><Plus className="mr-2 h-4 w-4" />Start linking</Button></div> : <ScrollArea className="h-[520px] pr-3"><div className="space-y-3">{groups.map((group) => <button key={group.id} type="button" onClick={() => setSelectedOwnerId(group.id)} className={cn('w-full rounded-2xl border p-4 text-left transition-colors', selectedOwnerId === group.id ? 'border-primary bg-primary/5' : 'border-border/60 bg-background hover:bg-muted/40')}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-foreground">{group.name}</p><p className="mt-1 text-xs text-muted-foreground">{group.email}</p></div><Badge className={cn('rounded-full border px-2.5 py-1 text-xs', group.attention > 0 ? 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300' : 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-300')}>{group.attention > 0 ? `${group.attention} attention` : 'Stable'}</Badge></div><div className="mt-4 flex items-center justify-between text-xs text-muted-foreground"><span>{group.active} active clients</span><span>{relativeTime(group.latest)}</span></div></button>)}</div></ScrollArea>}
          </div>
        </aside>
        ...
        <section className="rounded-lg border bg-card p-5 text-card-foreground shadow-sm sm:p-6">
          {loading ? <div className="space-y-4"><Skeleton className="h-10 w-64" /><div className="grid gap-3 lg:grid-cols-3"><Skeleton className="h-24 rounded-2xl" /><Skeleton className="h-24 rounded-2xl" /><Skeleton className="h-24 rounded-2xl" /></div><Skeleton className="h-12 rounded-2xl" /><Skeleton className="h-32 rounded-3xl" /></div> : !selectedGroup ? <div className="flex min-h-[420px] flex-col items-center justify-center rounded-3xl border border-dashed text-center"><Link2 className="h-12 w-12 text-muted-foreground" /><h3 className="mt-4 text-lg font-semibold">Select an owner group</h3><p className="mt-2 max-w-md text-sm text-muted-foreground">Choose an owner from the list to inspect linked clients, update shared access, or reactivate relationships.</p></div> : <div className="space-y-6">
            <div className="border-b border-border pb-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <h3 className="text-2xl font-semibold tracking-tight text-foreground">{selectedGroup.name}</h3>
              ...
              <div className="mt-5 grid gap-3 lg:grid-cols-3">{[['Relationships', selectedGroup.links.length, 'Total linked clients for this owner'], ['Active', selectedGroup.active, 'Clients currently sharing live data'], ['Attention', selectedGroup.attention, 'Inactive or suspended links']].map(([label, value, note], index) => <div key={String(label)} className={cn('rounded-2xl border p-4', index === 1 ? 'border-emerald-200/80 bg-emerald-50/70 dark:border-emerald-900/60 dark:bg-emerald-950/20' : index === 2 ? 'border-amber-200/80 bg-amber-50/70 dark:border-amber-900/60 dark:bg-amber-950/20' : 'border-border/60 bg-muted/30')}><p className="text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">{label}</p><p className="mt-3 text-2xl font-semibold text-foreground">{value}</p><p className="mt-1 text-sm text-muted-foreground">{note}</p></div>)}</div>
            </div>
