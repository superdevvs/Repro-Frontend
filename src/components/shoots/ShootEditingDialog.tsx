    ...
    <DialogContent className="flex max-h-[90dvh] max-w-3xl flex-col overflow-hidden">
      ...
      <div className="min-h-0 space-y-5 overflow-y-auto pr-1">
        ...
        {plan && <>
          ...
          {mode === 'ai' && <div className="rounded-lg bg-muted/40 p-3 text-sm">{allPhotos ? `Full Shoot · Fotello · all ${plan.photos.length} photos` : `Selected photos · Autoenhance · ${fileIds?.length} photos`}<p className="mt-1 text-muted-foreground">Finished AI photos appear in Edited, in their matching categories, without a review step.</p></div>}
