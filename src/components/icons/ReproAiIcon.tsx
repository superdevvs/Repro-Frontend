import React from 'react';

export const REPRO_AI_ICON_PATH = 'M74.0137 30.8125V79.75H8.76367V30.8125L41.3887 7.25L74.0137 30.8125ZM40.0312 29.8799C38.5156 37.268 32.5833 42.9459 25.1357 44.1357L19.6387 45.0137L25.1357 45.8916C32.5832 47.0815 38.5155 52.7595 40.0312 60.1475L41.3887 66.7637L42.7461 60.1475C44.2618 52.7595 50.1942 47.0815 57.6416 45.8916L63.1387 45.0137L57.6416 44.1357C50.1941 42.9458 44.2618 37.2679 42.7461 29.8799L41.3887 23.2637L40.0312 29.8799ZM41.3887 40.0186C42.759 41.9179 44.4046 43.6014 46.2666 45.0137C44.4046 46.4259 42.759 48.1093 41.3887 50.0088C40.0182 48.1092 38.372 46.426 36.5098 45.0137C38.3721 43.6013 40.0182 41.9182 41.3887 40.0186Z';

interface ReproAiIconProps {
  className?: string;
  useSolid?: boolean;
}

export function ReproAiIcon({ className, useSolid = false }: ReproAiIconProps) {
  return (
    <svg 
      width="87" 
      height="87" 
      viewBox="0 0 87 87" 
      fill="none" 
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <path 
        d={REPRO_AI_ICON_PATH}
        fill={useSolid ? "currentColor" : "url(#paint0_linear_213_586)"}
      />
      {!useSolid && (
        <defs>
          <linearGradient 
            id="paint0_linear_213_586" 
            x1="41.3887" 
            y1="7.25" 
            x2="41.3887" 
            y2="79.75" 
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor="#1C48E9"/>
            <stop offset="1" stopColor="#92AAFF"/>
          </linearGradient>
        </defs>
      )}
    </svg>
  );
}







