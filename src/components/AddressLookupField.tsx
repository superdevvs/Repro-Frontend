...
const AddressLookupField: React.FC<AddressLookupFieldProps> = ({
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {sqftNotice && !error && (
        <p className="mt-1 flex items-center rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
        ...
        >
          {suggestions.map((suggestion) => (
            <button
              ...
              className="w-full px-3 py-2 text-left hover:bg-accent hover:text-accent-foreground focus-visible:bg-accent focus-visible:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring border-b border-border last:border-b-0"
            >
      ...
      {showSuggestions && suggestions.length === 0 && inputValue.length >= 1 && !isLoading && (
        <div className="absolute z-50 w-full mt-1 bg-popover text-popover-foreground border border-border rounded-md shadow-lg p-3">
