...
export function PickerPhotoCard({ tile, selected, onSelect, onRetry }: { tile: PickerTile & { error?: string }; selected: boolean; onSelect: (media: V4Media) => void; onRetry: () => void }) {
  ...
  return <article className="min-w-0">
    <button disabled={!tile.media} onClick={() => tile.media && onSelect(tile.media)} aria-label={`Select ${name}`} aria-pressed={selected} className="group block w-full min-w-0 text-left disabled:cursor-wait">
      <div className={`relative aspect-[3/2] overflow-hidden rounded-lg border-2 bg-muted/30 ${selected ? 'border-primary' : 'border-transparent'}`}>
      ...
      <p className="mt-1 truncate text-[11px]" title={name}>{name}</p>
    </button>
    {stacked && <div className="mt-1 flex gap-0.5" aria-label="Bracket exposures">{tile.sources.map(source => <StudioImage key={source.id} src={source.thumbnailUrl} alt={source.filename} title={source.filename} className="h-6 min-w-0 flex-1 rounded-sm object-cover" />)}</div>}
