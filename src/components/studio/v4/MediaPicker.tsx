...
import { ArrowLeft, CheckCheck, ChevronRight, FolderOpen, Search, UploadCloud, X } from 'lucide-react';
  ...
  return <Dialog open={open} onOpenChange={v => !v && onClose()}><DialogContent aria-describedby={undefined} className="v4-media-dialog flex h-[min(900px,94dvh)] w-[96vw] max-w-7xl flex-col gap-0 overflow-hidden rounded-xl p-0 [&>button:last-child]:top-3">
    ...
    <section className="flex min-h-0 min-w-0 flex-1 flex-col" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); void upload(e.dataTransfer.files); }}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2 sm:p-3">
          ...
          {!loading && !error && !(active ? tiles : shoots).length && <div className="flex min-h-48 flex-col items-center justify-center gap-3 text-center"><EmptyState icon={!active && search.trim() ? "search" : "photos"} title={<>{active ? `No compatible ${tab} media in this shoot.` : search.trim() ? 'No shoots match your search.' : 'No shoots are available to your account yet.'}</>} size="compact" />{!active && search.trim() ? <Button variant="outline" onClick={() => setSearch('')}>Clear Search</Button> : <Button variant="outline" onClick={() => fileInput.current?.click()}>Upload media</Button>}</div>}
        </div>
    </section>
