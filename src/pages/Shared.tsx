...
export default function Shared() {
  ...
  const selectedClientId = searchParams.get('client') || searchParams.get('owner') || '';

  const visibility = useLinkedSharedVisibility();

  const clientsQuery = useQuery({
    queryKey: ['shared-linked-clients', user?.id ?? null],
    queryFn: ({ signal }) => fetchMyLinkedClients(signal),
    enabled: visibility.data.hasLinkedAccounts,
    staleTime: 60 * 1000,
    retry: 1,
  });

  const clients = clientsQuery.data?.linkedAccounts ?? visibility.data.linkedAccounts ?? [];
  ...
  useEffect(() => {
    ...
    nextParams.set('client', clients[0].id);
    setSearchParams(nextParams, { replace: true });
  }, [clients, searchParams, selectedClientId, setSearchParams]);

  const sharedQuery = useQuery({
    queryKey: ['shared-client-data', user?.id ?? null, selectedClientId],
    queryFn: ({ signal }) => fetchMyLinkedClientSharedData(selectedClientId, signal),
    enabled: Boolean(selectedClientId) && visibility.data.hasLinkedAccounts,
    staleTime: 30 * 1000,
    retry: 1,
  });
  ...
  const clientData = sharedQuery.data;
  const invoices = clientData?.sharedData.invoiceHistory ?? [];
        ...
        <section className="rounded-[28px] border border-slate-200/70 bg-[radial-gradient(circle_at_top_left,_rgba(79,168,255,0.16),_transparent_32%),linear-gradient(180deg,_rgba(255,255,255,0.94),_rgba(248,250,252,0.96))] p-5 shadow-sm shadow-slate-950/5 dark:border-slate-800 dark:bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.16),_transparent_30%),linear-gradient(180deg,_rgba(2,6,23,0.92),_rgba(15,23,42,0.96))] sm:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-4 text-2xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-3xl">
                View shared linked client activity.
              </h1>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                Switch linked clients to inspect their shared shoots, invoices, and enabled access categories.
              </p>
            </div>
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-4">
            <div className="rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/70">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">Linked client</p>
              <p className="mt-3 truncate text-lg font-semibold text-slate-900 dark:text-white">
                {selectedClient?.name || 'No client selected'}
              </p>
              <p className="mt-1 truncate text-sm text-muted-foreground">{selectedClient?.email || 'Pick a linked client'}</p>
            </div>
            <div className="rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/70">
              ...
              <p className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">
                {clientData?.sharedData.totalShoots ?? 0}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">Client-scoped total</p>
            </div>
            <div className="rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/70">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">Invoices</p>
              <p className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">
                {currencyFormatter.format(clientData?.sharedData.totalSpent ?? 0)}
              </p>
            ...
            <div className="rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/70">
              ...
              <p className="mt-3 text-lg font-semibold tracking-tight text-slate-900 dark:text-white">
                {formatRelative(clientData?.sharedData.lastActivity)}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">{formatDateTime(clientData?.sharedData.lastActivity)}</p>
            </div>
          </div>
        </section>
        ...
        {sharedQuery.isLoading ? (
          ...
          <Card className="rounded-3xl border-red-200 bg-red-50 dark:border-red-950/60 dark:bg-red-950/20">
            <CardContent className="flex items-start gap-3 p-5 text-sm text-red-700 dark:text-red-300">
              ...
              <div>
                <p className="font-medium">Unable to load shared data for this client.</p>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_360px]">
            <div className="space-y-6">
              {clientData.link.sharedDetails.shoots && (
                <Card className="rounded-3xl border-slate-200/70 dark:border-slate-800">
                  <CardHeader className="pb-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Latest shoot activity shared by {clientData.client.name}.
                    </p>
                  </CardHeader>
                          ...
                          >
                            <div className="flex min-w-0 gap-4">
                              <div className="h-16 w-24 overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-900">
                                {heroImage ? (
                                  <img src={heroImage} alt={shoot.address} className="h-full w-full object-cover" loading="lazy" />
                                ) : (
              ...
              {clientData.link.sharedDetails.invoices && (
                <Card className="rounded-3xl border-slate-200/70 dark:border-slate-800">
                  <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2 text-lg">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Invoice activity shared by this client.
                    </p>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {invoices.length === 0 ? (
                        ...
                        <div key={invoice.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200/70 p-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              ...
                              {invoice.balanceDue > 0 ? ` · ${currencyFormatter.format(invoice.balanceDue)} due` : ''}
                              {invoice.dueDate ? ` · Due ${invoice.dueDate}` : ''}
                              {invoice.shoot?.address ? ` · ${invoice.shoot.address}` : ''}
                            </p>
                          </div>
                          <Badge variant="outline" className="w-fit rounded-full px-3 py-1 capitalize">
                            {invoice.status}
              ...
              {clientData.link.sharedDetails.shoots && (
                <Card className="rounded-3xl border-slate-200/70 dark:border-slate-800">
                  <CardHeader className="pb-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Property activity associated with this client’s shared shoots.
                    </p>
                  </CardHeader>
              ...
              <Card className="rounded-3xl border-slate-200/70 dark:border-slate-800">
                ...
                <CardContent className="space-y-4">
                  <div className="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/50">
                    <p className="text-xl font-semibold text-slate-900 dark:text-white">{clientData.client.name}</p>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Badge variant="outline" className="rounded-full px-3 py-1">
                        Linked {formatRelative(clientData.link.linkedAt)}
                      </Badge>
                    </div>
                  </div>
